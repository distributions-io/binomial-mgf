...
function validate( opts, options ) {
	...
	if ( options.hasOwnProperty( 'n' ) ) {
		...
		if ( !isNonNegativeInteger( opts.n ) ) {
			return new TypeError( 'mgf()::invalid option. `n` parameter must be a non-negative integer. Option: `' + opts.n + '`.' );
		}
	}
	if ( options.hasOwnProperty( 'p' ) ) {
		opts.p = options.p;
		if ( !isNumber( opts.p ) ) {
			return new TypeError( 'mgf()::invalid option. `p` parameter must be a number primitive. Option: `' + opts.p + '`.' );
		}
	}
